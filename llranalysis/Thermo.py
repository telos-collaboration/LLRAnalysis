...
def find_critical_region(E, b):
    ...
    for i in range(len(b)):
        ...
        if len(roots >= 2):
            ...
            cr.append(False)
    mini = min(np.nonzero(cr)[0])
    maxi = max(np.nonzero(cr)[0])
    midb = (b[mini] + b[maxi]) / 2.0
    ...
    [maxi, midi, mini] = np.sort(
        np.array(
            [(mini, E[mini]), (midi, E[midi]), (maxi, E[maxi])],
            dtype=[("ind", int), ("temp", float)],
        ),
        order="temp",
    )["ind"]
    [meta_mini, meta_maxi] = np.sort(
        np.array(
            [(meta_mini, E[meta_mini]), (meta_maxi, E[meta_maxi])],
            dtype=[("ind", int), ("temp", float)],
        ),
        order="temp",
    )["ind"]
...
def thermodynamics(boot_folder, n_repeats, Es):
    ...
    lenx = len(Es)
    leny = n_repeats
    S = S.reshape([leny, lenx])
    T = T.reshape([leny, lenx])
    F = F.reshape([leny, lenx])
    U = U.reshape([leny, lenx])
    return S, T, F, U
...
def free_energy(
    ...
    S, T, F, U = thermodynamics(
        boot_folder, n_repeats, np.linspace(Ep.min(), Ep.max(), 100)
    )
    F /= V
    Sigma = S.mean() / V
    S_int = S.mean(axis=0)
    ...
    F_int = F.mean(axis=0)
    U_int = U.mean(axis=0)
    S_int_err = error.calculate_error_set(S, num_samples, error_type)
    T_int_err = error.calculate_error_set(T, num_samples, error_type)
    F_int_err = error.calculate_error_set(F, num_samples, error_type)
    U_int_err = error.calculate_error_set(U, num_samples, error_type)
    ...
    plt.plot(T_int[: maxi + 1], F_red_int_avr[: maxi + 1], "k-")
    plt.plot(T_int[mini:], F_red_int_avr[mini:], "k-")
    plt.plot(T_int[maxi : meta_mini + 1], F_red_int_avr[maxi : meta_mini + 1], "b-")
    plt.plot(T_int[meta_maxi : mini + 1], F_red_int_avr[meta_maxi : mini + 1], "b-")
    plt.plot(T_int[meta_mini : midi + 1], F_red_int_avr[meta_mini : midi + 1], "r-")
    plt.plot(T_int[midi : meta_maxi + 1], F_red_int_avr[midi : meta_maxi + 1], "r-")
    ...
    for c, (x, x_err, y, y_err) in enumerate(
        zip(T_avr[inds], T_err[inds], F_red_avr[inds], F_red_err[inds])
    ):
        plt.errorbar(
            x,
            y,
    ...
    axsins = inset_axes(
        ...
        height=3,
        loc="upper left",
        bbox_to_anchor=(0.55, 1 - 0.3, 0.3, 0.3),
        bbox_transform=ax.transAxes,
    )

    axsins.plot((6 * V - U_int[: maxi + 1]) / (6 * V), 1 / T_int[: maxi + 1], "k-")
    plt.plot((6 * V - U_int[mini:]) / (6 * V), 1 / T_int[mini:], "k-")
    axsins.plot(
        (6 * V - U_int[maxi : meta_mini + 1]) / (6 * V),
        1 / T_int[maxi : meta_mini + 1],
        "b-",
    )
    plt.plot(
        (6 * V - U_int[meta_maxi : mini + 1]) / (6 * V),
        1 / T_int[meta_maxi : mini + 1],
        "b-",
    )
    axsins.plot(
        (6 * V - U_int[meta_mini : midi + 1]) / (6 * V),
        1 / T_int[meta_mini : midi + 1],
        "r-",
    )
    plt.plot(
        (6 * V - U_int[midi : meta_maxi + 1]) / (6 * V),
        1 / T_int[midi : meta_maxi + 1],
        "r-",
    )
    Emax = max(
        (6 * V - U_int[meta_maxi]) / (6 * V), ((6 * V - U_int[meta_mini]) / (6 * V))
    )
    axsins.set_ylim([1 / T_int[maxi - 1], 1 / T_int[mini + 1]])
    axsins.set_xlim(
        [(6 * V - U_int[maxi - 2]) / (6 * V), (6 * V - U_int[mini + 2]) / (6 * V)]
    )

    axsins.set_xlabel("$u_p$", fontsize=30)
    axsins.set_ylabel("$a_n$", fontsize=30)
    for c, (x, y, y_err) in enumerate(
        zip((6 * V - U_avr[inds]) / (6 * V), ak[inds], ak_err[inds])
    ):
        axsins.errorbar(
            x, y, y_err, color=cols(c / (len(ak[inds]) - 1)), marker="o", ms=markersize
        )
    plt.show()


def plot_ak_dist_potential(
    boot_folder,
    n_repeats,
    beta,
    ulim,
    ...
    for nr in range(n_repeats):
        ...
        xs = np.append(xs, x)
        ys = np.append(ys, y)
        a = np.append(a, final_df["a"].values)
        Eks = np.append(Eks, final_df["Ek"].values)
    xs.shape = [n_repeats, len(x)]
    ys.shape = [n_repeats, len(y)]
    a.shape = [n_repeats, len(final_df["a"].values)]
    Eks.shape = [n_repeats, len(final_df["Ek"].values)]
    xs = xs.mean(axis=0)
    ys = ys.mean(axis=0)
    a_err = error.calculate_error_set(a, num_samples, error_type)
    a = a.mean(axis=0)
    Eks = Eks.mean(axis=0)  # / (6*V);
    fig, axs = plt.subplots(
        3, 1, figsize=(10, 18), gridspec_kw={"height_ratios": [5, 5, 5]}
    )
    Emax = ulim[1]
    Emin = ulim[0]
    ...
    for s in spl:
        axs[0].plot([(s / (6 * V)), (s / (6 * V))], [beta, blim[0]], "m--")
    ...
    for s, ym in zip(spl, ymax):
        axs[1].plot([s / (6 * V), s / (6 * V)], [1, 0], ls="--", c="m")
    ...
    ymax = f(spl)
    for s, ym in zip(spl, ymax):
        axs[2].plot([s / (6 * V), s / (6 * V)], [100, ym], ls="--", c="m")
...
def plot_fxa_polyakovloop_critical(
    boot_folder,
    n_repeats,
    selected_repeat,
    plt_all=True,
    ...
    stable_inds = ((U_avr >= U_int[mini]) * (U_avr <= U_int[meta_maxi])) + (
        (U_avr >= U_int[meta_mini]) * (U_avr <= U_int[maxi])
    )
...
def free_energy_difference(
    ...
    F_deconf = np.interp(
        T_cri, T_int[deconf_inds][::-1], F_red_int_avr[deconf_inds][::-1]
    )
