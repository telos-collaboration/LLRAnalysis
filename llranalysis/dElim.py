...
def compare_dE_plot_a(boot_folders, n_repeats, blim,ulim,num_samples=200, error_type = 'standard deviation'):
    ...
    for bf, nr in zip(boot_folders, n_repeats):
        ...
        ax1.errorbar(Eks/(6*V),aks_mean,aks_error, fmt = colours[j] + '-', label = '$\Delta_{u_p}$' + f'= {2*dE / (6*V) :.4f}')
        ax2.errorbar(Eks/(6*V),aks_mean,aks_error, fmt = colours[j] + '-', label = '$\Delta_{u_p}$' + f'= {2*dE / (6*V) :.4f}')
        j += 1
    ax1.set_ylabel('$a_n$')
    ax2.set_ylabel('$a_n$')
