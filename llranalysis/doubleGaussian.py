...
def double_Gaussian_velocity(x, amp1, mn1, std1, amp2, mn2, std2):
    G1 = np.exp(-((x - mn1) ** 2.0) / (2 * (std1**2.0))) * amp1
    G2 = np.exp(-((x - mn2) ** 2.0) / (2 * (std2**2.0))) * amp2
    return G1 * ((mn1 - x) / std1**2.0) + G2 * ((mn2 - x) / std2**2.0)


def double_Gaussian_acceleration(x, amp1, mn1, std1, amp2, mn2, std2):
    G1 = np.exp(-((x - mn1) ** 2.0) / (2 * (std1**2.0))) * amp1
    G2 = np.exp(-((x - mn2) ** 2.0) / (2 * (std2**2.0))) * amp2
    return (G1 / std1**2.0) * (((mn1 - x) / std1) ** 2.0 - 1.0) + (G2 / std2**2.0) * (
        ((mn2 - x) / std2) ** 2.0 - 1.0
    )
...
def double_Gaussian_find_peaks(x, amp1, mn1, std1, amp2, mn2, std2):
    G1 = np.exp(-((x - mn1) ** 2.0) / (2 * (std1**2.0))) * amp1
    G2 = np.exp(-((x - mn2) ** 2.0) / (2 * (std2**2.0))) * amp2
    y_vel = G1 * ((mn1 - x) / std1**2.0) + G2 * ((mn2 - x) / std2**2.0)
    y_acc = (G1 / std1**2.0) * (((mn1 - x) / std1) ** 2.0 - 1.0) + (G2 / std2**2.0) * (
        ((mn2 - x) / std2) ** 2.0 - 1.0
    )
    x_mid = x[y_acc > 0][np.argmin(np.abs(y_vel[y_acc > 0]))]
    y_mid = (G1 + G2)[y_acc > 0][np.argmin(np.abs(y_vel[y_acc > 0]))]
    indx = [
        np.argmin(np.abs(y_vel[(y_acc < 0) * (x < x_mid)])),
        np.argmin(np.abs(y_vel[(y_acc < 0) * (x > x_mid)])),
    ]
    y = np.array(
        [
            (G1 + G2)[(y_acc < 0) * (x < x_mid)][indx[0]],
            (G1 + G2)[(y_acc < 0) * (x > x_mid)][indx[1]],
            y_mid,
        ]
    )
    x = np.array(
        [
            x[(y_acc < 0) * (x < x_mid)][indx[0]],
            x[(y_acc < 0) * (x > x_mid)][indx[1]],
            x_mid,
        ]
    )
    return x, y
...
def find_critical_point(betas, dB, tol, N, final_df, full_obs, folder):
    ...
    try:
        ...
        xs_tmp = np.linspace(
            np.min(x[y > (0.5 * (max(y) / N))]),
            np.max(x[y > (0.5 * (max(y) / N))]),
            1000,
        )
        ...
        xopt, xcov = curve_fit(double_Gaussian, x, y, p0=estimates)
    j = 0.0
    ys_tmp = double_Gaussian(
        xs_tmp, xopt[0], xopt[1], xopt[2], xopt[3], xopt[4], xopt[5]
    )
    x, y = llr.calc_prob_distribution(final_df, Bc, lnz)
    b, a = double_Gaussian_find_peaks(
        xs_tmp, xopt[0], xopt[1], xopt[2], xopt[3], xopt[4], xopt[5]
    )
    val = (a[0] - N * a[1]) / a[0]
    ...
    while not np.isclose(abs(val), 0, atol=tol):
        ...
        if low_val * up_val >= 0:
            Bc = Bc - (
                np.sign(val) * np.sign(b[0] - b[1]) * dB
            )  # Finding extremes of Bisection
        else:
            Bc = (low_b + up_b) / 2  # Bisection
        lnz = llr.calc_lnZ(final_df["Ek"].values, final_df["a"].values, Bc)
        x, y = llr.calc_prob_distribution(final_df, Bc, lnz, xs_tmp)
        xopt, xcov = curve_fit(
            double_Gaussian,
