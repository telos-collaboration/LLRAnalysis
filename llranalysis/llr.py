...
def ReadRep(file, new_rep, poly):
                ...
                # Append a and S0
                    ...
                    for word in re.split("\s", line):
                        if utils.check_float(word):
                            ...
                            rep.append(new_rep)
                            V.append(new_V)
                            rm_plaq.append(new_plaq * 6.0 * new_V)
                            Lt.append(new_Lt)
                # End of RM
    ...
    final = pd.DataFrame(
        data={"a": a[-1], "Ek": S0[-1], "dE": dS0[-1], "V": V[-1], "Lt": new_Lt},
        index=[new_rep],
    )
...
def CSV(files, folder, poly=True):
    exists = (
        os.path.isfile(folder + "RM.csv")
        and os.path.isfile(folder + "fa.csv")
        and os.path.isfile(folder + "final.csv")
    )
    if exists:
...
def calc_observables_full(betas, final_df, fa_df, calc_poly=True):
    ...
    for beta in betas:
        ...
        E2.append((calc_E2(Eks, aks, beta)) / ((6 * V) ** 2.0))
        E4.append((calc_E4(Eks, aks, beta)) / ((6 * V) ** 4.0))
        if len(fa_df["S"]) > 0 and calc_poly:
            lnz_obs = calc_fxa_lnZ(fa_df, final_df, beta)
            poly.append(calc_fxa_obs(fa_df, final_df, beta, "Poly", 1.0, lnz_obs))
            poly_sq.append(calc_fxa_obs(fa_df, final_df, beta, "Poly", 2.0, lnz_obs))
            poly_4.append(calc_fxa_obs(fa_df, final_df, beta, "Poly", 4.0, lnz_obs))
        else:
            poly.append(0.0)
            poly_sq.append(0.0)
            poly_4.append(0.0)
    SH = (np.array(E2) - (np.array(E) ** 2.0)) * 6.0 * V
    Xlp = (np.array(poly_sq) - (np.array(poly) ** 2.0)) * (V / Lt)
    Blp = 0.0
    binder = 1 - (np.array(E4) / (3 * (np.array(E2) ** 2.0)))
    if len(poly_sq) > 0:
        if min(poly_sq) != 0.0:
            Blp = 1.0 - (np.array(poly_4) / (3 * (np.array(poly_sq) ** 2.0)))
    return pd.DataFrame(
        data={
            "b": betas,
            "u": E,
            ...
            "Xlp": Xlp,
            "Blp": Blp,
            "V": V * np.ones(len(poly)),
            "Lt": Lt * np.ones(len(poly)),
        }
    )
...
def calc_lnZ(Eks, aks, beta, rmpf=False):
    ...
    for Ek, a in zip(Eks, aks):
        A = mp.mpf(a + beta)
        if not (A == mp.mpf(0.0)):
            full_exp = mp.exp(pi_exp + (Ek * beta) + a * (dE / 2.0))
            T = A * dE / 2.0
            shn = mp.sinh(T)
            Z += full_exp * shn / A
        else:
            print("A=0, beta =", beta, ", a=", a)
            full_exp = mp.exp(pi_exp - (Ek * a) + a * (dE / 2.0))
            Z += (dE / 2.0) * full_exp
        pi_exp += a * dE
    ln_Z = mp.ln(2 * Z)
    if not rmpf:
...
def calc_lnrho(final_df, E):
    ...
    final_df = final_df.sort_values(by=["Ek"], ignore_index=True)
    for En, an in zip(
        final_df["Ek"].values - (final_df["dE"].values / 2), final_df["a"].values
    ):
        if E >= En and E < (En + dE):
...
def calc_E(Eks, aks, beta):
    ...
    for Ek, a in zip(Eks, aks):
        ...
        if not (A == mp.mpf(0.0)):
            ...
            shn = mp.sinh(T)
            Z += full_exp * shn / A
            E += full_exp * ((Ek - (1.0 / A)) * shn + (dE / 2.0) * mp.cosh(T)) / A
        else:
            print("A=0, beta =", beta, ", a=", a)
            full_exp = mp.exp(pi_exp - (Ek * a) + a * (dE / 2.0))
            E += full_exp * (dE / 2.0) * Ek
            Z += (dE / 2.0) * full_exp
        pi_exp += a * dE
    return E / Z
...
def calc_E2(Eks, aks, beta):
    ...
    for Ek, a in zip(Eks, aks):
        ...
        if not (A == mp.mpf(0.0)):
            full_exp = (
                2.0 * mp.exp(pi_exp + (beta * (Ek - (dE / 2.0))) + (A * dE / 2.0)) / A
            )
            T = A * dE / 2.0
            E2 += full_exp * (
                mp.sinh(T)
            ...
            E2 += (
                2
                * full_exp
                * (((Ek**2.0) * dE / 2.0) + ((1.0 / 3.0) * ((dE / 2.0) ** 3.0)))
            )
        pi_exp += a * dE
    # print(E / Z)
    return E2
    ...
    for Ek, a in zip(Eks, aks):
        ...
        if not (A == mp.mpf(0.0)):
            ...
            full_exp = (
                np.math.factorial(N)
                * 2.0
                * mp.exp(pi_exp + (beta * (Ek - (dE / 2.0))) + (A * dE / 2.0))
            )
            T = A * dE / 2.0
            for n in range(N + 1):
                snh_term = mp.mpf(0)
                for i in range(int(np.floor(n / 2.0)) + 1):
                    snh_term += (
                        ((dE / 2.0) ** (2 * i))
                        * (Ek ** (n - 2 * i))
                        / (np.math.factorial(2 * i) * np.math.factorial(n - 2 * i))
                    )
                ch_term = mp.mpf(0)
                if n > 0:
                    for i in range(1, int(np.ceil(n / 2.0)) + 1):
                        ch_term += (
                            ((dE / 2.0) ** (2 * i - 1))
                            * (Ek ** (n - 2 * i + 1))
                            / (
                                np.math.factorial(2 * i - 1)
                                * np.math.factorial(n - 2 * i + 1)
                            )
                        )
                EN += (
                    (snh_term * mp.sinh(T) + ch_term * mp.cosh(T))
                    * full_exp
                    * ((-1.0) ** (N - n))
                    * (mp.power(A, (n - N - 1)))
                )
...
def calc_fxa_lnZ(fxa_df, final_df, beta):
    dE = final_df["dE"][0]
    final_df = final_df.sort_values(by=["Ek"], ignore_index=True)
    Ek = final_df["Ek"].values[0]
    a = final_df["a"].values[0]
    ...
    for Ek, a in zip(final_df["Ek"].values, final_df["a"].values):
        ...
        ln_rhok = calc_lnrho(final_df, Ek)
        VEV_exp = (
            ((a + beta) * S - a * (Ek) + ln_rhok) - np.log(len(S)) + np.log(dE) - vmax
        )
        for ve in VEV_exp:
            Z += mp.exp(mp.mpf(ve))
...
def calc_prob_distribution(final_df, beta, lnz, xs=np.array([])):
    if xs.shape[0] == 0:
        xs = np.linspace(
            np.min(final_df["Ek"].values + final_df["dE"].values)
            / (6 * final_df["V"].values[0]),
            np.max(final_df["Ek"].values) / (6 * final_df["V"].values[0]),
            1000,
        )
        # print('Here')
    ys = np.zeros(xs.size)
    for x, i in zip(xs, range(len(xs))):
        ys[i] = np.exp(
            calc_lnrho(final_df, x * (6 * final_df["V"].values[0]))
            + beta * x * (6 * final_df["V"].values[0])
            - lnz
        )
...
def prepare_data(
    ...
    for nr in range(n_repeats):
        ...
        critical_dF = ReadObservables(
            betas_critical,
            final_df,
            fa_df,
            f"{LLR_folder}{nr}/CSV/",
            file="obs_critical.csv",
            calc_poly=False,
        )
...
def half_intervals(originalfolder, reducedfolder, mode="even"):
    for i in range(20):
        ...
        if mode == "even":
            Eks = np.sort(np.unique(FNL["Ek"].values))[0:-1:2]
        elif mode == "odd":
            Eks = np.sort(np.unique(FNL["Ek"].values))[1::2]
        elif mode == "mean":
            Eks = (
                (np.sort(np.unique(FNL["Ek"].values))[0:-1:2])
                + (np.sort(np.unique(FNL["Ek"].values))[1::2])
            ) / 2
        RM_red = pd.DataFrame()
        FA_red = pd.DataFrame()
        FNL_red = pd.DataFrame()
        dE = Eks[1] - Eks[0]
        for ek in Eks:
...
def free_energy_df(boot_folder, n_repeats, num_samples, error_type):
    ...
    Sigma = S.mean()
    F = (F + Sigma * T) / V
    mini, meta_mini, midi, meta_maxi, maxi = thermo.find_critical_region(
        6 * V - U.mean(axis=0), T.mean(axis=0)
    )
    ulim_ind = [max([maxi - 2, 0]), min([mini + 2, len(Ep) - 1])]
    ulim_int = [
        1 - (U.mean(axis=0)[ulim_ind[0]] / (6 * V)),
        1 - (U.mean(axis=0)[ulim_ind[1]] / (6 * V)),
    ]
    S_int, T_int, F_int, U_int = thermo.thermodynamics(
        boot_folder,
        n_repeats,
        np.linspace((6 * V) * ulim_int[0], (6 * V) * ulim_int[1], 1000),
    )
    F_int = (F_int + Sigma * T_int) / V

    mini, meta_mini, midi, meta_maxi, maxi = thermo.find_critical_region(
        6 * V - U_int.mean(axis=0), T_int.mean(axis=0)
    )
    ...
    for i in range(n_repeats):
        try:
            ...
            t = np.linspace(
                max(min(T_int[i, meta_h]), min(T_int[i, meta_c])),
                min(max(T_int[i, meta_h]), max(T_int[i, meta_c])),
                10000,
            )
            tc_ind = np.where(
                T_int[i, :]
                == T_int[i, np.append(meta_h, meta_c)][
                    np.argmin(
                        abs(
                            T_int[i, np.append(meta_h, meta_c)]
                            - t[np.argmin(abs(f(t) - g(t)))]
                        )
                    )
                ]
            )[0]
            ...
            print(
                "Error no overlap in two meta-stable branches, likely misclassified : including unstable branch"
            )
            mc = meta_c
            mh = np.append(meta_h, unstable).flatten()
            f = interp1d(T_int[i, mh], F_int[i, mh])
            g = interp1d(T_int[i, mc], F_int[i, mc])
            t = np.linspace(
                max(min(T_int[i, mh]), min(T_int[i, mc])),
                min(max(T_int[i, mh]), max(T_int[i, mc])),
                10000,
            )
            tc_ind = np.where(
                T_int[i, :]
                == T_int[i, np.append(mh, mc)][
                    np.argmin(
                        abs(
                            T_int[i, np.append(mh, mc)] - t[np.argmin(abs(f(t) - g(t)))]
                        )
                    )
                ]
            )[0]
        ...
        up_min = np.append(
            up_min,
            1 - (U_int[i, meta_h][np.argmin(abs(T_int[i, meta_h] - tmin))] / (6.0 * V)),
        )
        up_max = np.append(
            up_max,
            1 - (U_int[i, meta_c][np.argmin(abs(T_int[i, meta_c] - tmin))] / (6.0 * V)),
        )
        ...
        xs = np.append(xs, x)
        ys = np.append(ys, y * (6 * V))

    xs.shape = [n_repeats, len(x)]
    ys.shape = [n_repeats, len(y)]
    ...
    sigma = (((Lt**3) * Ls * (P_min_F - P_max_F)) / (2 * Tc)) + (
        ((Lt**2) * np.log(Ls)) / (4 * (Ls**2))
    )
    ...
    F_int = F_int.mean(axis=0)
    T_int = T_int.mean(axis=0)
    S_int = S_int.mean(axis=0)
    U_int = U_int.mean(axis=0)
    F = F.mean(axis=0)
    T = T.mean(axis=0)
    S = S.mean(axis=0)
    U = U.mean(axis=0)
    Tc_err = error.calculate_error(Tc, num_samples, error_type)
    Tc = Tc.mean(axis=0)
    dF_err = error.calculate_error(P_min_F - P_max_F, num_samples, error_type)
    dF = (P_min_F - P_max_F).mean(axis=0)
    du_err = error.calculate_error(up_max - up_min, num_samples, error_type)
    du = (up_max - up_min).mean(axis=0)
    up_min_err = error.calculate_error(up_min, num_samples, error_type)
    up_min = up_min.mean(axis=0)
    up_max_err = error.calculate_error(up_max, num_samples, error_type)
    up_max = up_max.mean(axis=0)
    sigma_err = error.calculate_error(sigma, num_samples, error_type)
    sigma = sigma.mean()
    xs = xs.mean(axis=0)
...
def pre_dat(folder, V, up_min, up_max, N_intervals, betas, location):
    ...
    with open(location + "pre.dat", "w") as f:
        f.write(output)
...
def obs_boot(folder, n_repeat, num_samples, error_type):
    ...
    for j in range(n_repeat):
        ...
        u = np.append(u, df["u"])
        Cu = np.append(Cu, df["Cu"])
        Bv = np.append(Bv, df["Bv"])
        lp = np.append(lp, df["lp"])
        Xlp = np.append(Xlp, df["Xlp"])
        b = np.append(b, df["b"])
    len_b = len(df["b"])
    u.shape = [n_repeat, len_b]
    Cu.shape = [n_repeat, len_b]
    Bv.shape = [n_repeat, len_b]
    lp.shape = [n_repeat, len_b]
    Xlp.shape = [n_repeat, len_b]
    ...
    for j in range(n_repeat):
        ...
        b = np.append(b, df["b"])
        y = [
            df.iloc[i]["Xlp"] for i in range(len(df)) if not np.isnan(df.iloc[i]["Xlp"])
        ]
...
def final_boot(folder, n_repeat, num_samples, error_type):
    ...
    ).to_csv(folder + "CSV/final.csv", index=False)
...
def DG_boot(folder, n_repeat, num_samples, error_type):
    ...
    pd.DataFrame(
        ...
        index=[0],
    ).to_csv(folder + "CSV/DG.csv", index=False)
...
def prepare_all(folder):
    ...
    if not os.path.isfile(folder + "pre.dat"):
        ...
        pre_dat(
            info_df["std_folder"][0],
            info_df["V"][0],
            info_df["umin"][0],
            info_df["umax"][0],
            info_df["n_replicas"][0],
            eval(info_df["IS_b"][0]),
            folder,
        )
        return
    betas = np.linspace(
        ...
        eval(info_df["betas"][0])[2],
    )
    betas_critical = np.linspace(
        eval(info_df["betas_critical"][0])[0],
        eval(info_df["betas_critical"][0])[1],
        eval(info_df["betas_critical"][0])[2],
    )
    check_obs = (
        os.path.isfile(folder + "CSV/comparison.csv")
        * os.path.isfile(folder + "CSV/obs.csv")
        * os.path.isfile(folder + "CSV/obs_critical.csv")
        * os.path.isfile(folder + "CSV/final.csv")
        * os.path.isfile(folder + "CSV/critical.csv")
    )
    ...
    for i in range(info_df["n_repeats"][0]):
        check_obs *= (
            os.path.isfile(folder + f"{i}/CSV/comparison.csv")
            * os.path.isfile(folder + f"{i}/CSV/obs.csv")
            * os.path.isfile(folder + f"{i}/CSV/obs_critical.csv")
            * os.path.isfile(folder + f"{i}/CSV/final.csv")
        )
    ...
    if not check_obs:
        ...
        final_boot(
            folder,
            info_df["n_repeats"][0],
            info_df["num_samples"][0],
            info_df["error_type"][0],
        )
        obs_boot(
            folder,
            info_df["n_repeats"][0],
