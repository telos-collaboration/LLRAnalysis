...
def plot_RM_repeats(boot_folder, n_boots, interval):
    ...
    axsins = inset_axes(
        ...
        height=3,
        loc="upper left",
        bbox_to_anchor=(0.5, 1 - 0.4, 0.3, 0.3),
...
def plot_RM_swaps(boot_folder, repeat, cmap):
    ...
    for i, n in enumerate(np.unique(RM_df["Rep"])):
        ...
        plt.plot(
            np.arange(1, len(an) + 1),
            an,
            lw=1,
            c=cols(i / len(np.unique(RM_df["Rep"]))),
        )
...
def plot_DG(LLR_folder, selected_repeat):
    ...
    plt.plot(
        x,
        dg.double_Gaussian(x, xopt[0], xopt[1], xopt[2], xopt[3], xopt[4], xopt[5]),
        "m--",
    ...
    b, a = dg.double_Gaussian_find_peaks(
    ...
    plt.text(
        b[1] - (b[1] - b[0]) / 2,
        (a[0] * 1.06) / 2,
        "$\\Delta \\langle u_p \\rangle_{\\beta_c}$",
        va="baseline",
